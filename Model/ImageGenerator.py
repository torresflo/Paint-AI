...
class ImageGenerator:
    ...
    def loadModel(self, modelName: PretrainedModelName):
        if(self.m_modelName is None or self.m_modelName != modelName):
            ...
            self.m_pipeline = StableDiffusionImg2ImgPipeline.from_pretrained(self.m_modelName.value, torch_dtype=torch.float16)
    ...
    def generateImage(self, modelName: PretrainedModelName, prompt, image, numInferenceSteps=50, guidanceScale=7.5, strength=0.75, seed=0):
        self.loadModel(modelName)
        generator = torch.Generator(self.m_device).manual_seed(seed)
        image = self.m_pipeline(prompt, image=image, num_inference_steps=numInferenceSteps, guidance_scale=guidanceScale, strength=strength, generator=generator).images[0]
        return image
